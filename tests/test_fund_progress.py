...
class FundProgressTests(unittest.TestCase):
    def test_card_is_one_expander_with_summary_and_edit_body(self):
        ...
        with patch('fund_progress.st', ui):
            with fund_progress_card(7, fund):
                ui.text_input(t('buckets.name'))
        ui.expander.assert_called_once_with('**Japan Trip** *53.3%*  \n' + t('buckets.saved_amount', amount='800.00'))
        ui.caption.assert_called_once_with(t('buckets.to_go', percentage='53.3', remaining='700.00').split(' · ')[1])
    ...
    def test_partial_goal(self):
        self.assertEqual(goal_progress_copy(800, 1500),
                         ("€800.00 / €1,500.00", t('buckets.to_go', percentage='53.3', remaining='700.00')))

    def test_no_target(self):
        for target in (None, 0):
            self.assertEqual(goal_progress_copy(800, target), ("€800.00", t('ui.no_target_set')))

    def test_reached_and_exceeded(self):
        for balance, percent in ((1500, "100.0%"), (1800, "120.0%")):
            self.assertEqual(goal_progress_copy(balance, 1500)[1], t('buckets.reached', percentage=percent[:-1]))
    ...
    def test_invalid_values_have_no_percentage(self):
        for balance, target in ((-1, 100), (1, -100), (None, 100),
            ...
            self.assertIn(t('ui.progress_unavailable'), detail)
            self.assertNotIn("%", detail)

    def test_zero_balance(self):
        self.assertEqual(goal_progress_copy(0, 100)[1], t('buckets.to_go', percentage='0.0', remaining='100.00'))
